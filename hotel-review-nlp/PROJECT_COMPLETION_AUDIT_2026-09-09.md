...
**Προτεραιότητα: P0 για συμβατότητα δεδομένων.**
...
**Κατάσταση κατά τη σύνταξη:** Το πλήρες Booking CSV διατηρήθηκε επιπλέον ως
`data/raw/booking_reviews_515k.csv` με ίδιο SHA-256. Ενημερώθηκαν baseline config,
EDA, σχετική οδηγία QLoRA και τεκμηρίωση download ώστε τα δύο datasets να έχουν
διαφορετικά ονόματα. Το νέο μεγάλο αρχείο είναι ignored.

**Εκκρεμής τοπική εξαίρεση:** Το παλιό `data/raw/hotel_reviews.csv` παραμένει
κλειδωμένο για εγγραφή από άλλη εφαρμογή. Δεν αποκαταστάθηκε ακόμη το μικρό
658-row δείγμα σε αυτή τη διαδρομή. Ο συγχρονισμός των πηγαίων αρχείων και της
αναφοράς μπορεί να ολοκληρωθεί ανεξάρτητα, αλλά το working tree θα δείχνει
αυτή τη μία τροποποίηση δεδομένων μέχρι να κλείσει το αρχείο. Δεν πρέπει να
γίνει commit του μεγάλου CSV. Έχει ζητηθεί από τον χρήστη να το κλείσει·
δεν τερματίστηκε κάποια εφαρμογή του.

Μετά το κλείσιμο, με επαληθευμένα τα δύο αντίγραφα του Booking CSV, η αποκατάσταση
του tracked δείγματος από το Git καταργεί τη διαφορά. Το annotation/load-test
schema σε αυτή τη διαδρομή παραμένει προσωρινά μπλοκαρισμένο.
